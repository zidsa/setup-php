...
- `PHP 5.6` to `PHP 7.4` are supported on multiple architecture on `Ubuntu`.
- For this, you can use `shivammathur/node` images as containers which have compatible `Nodejs` and `spc` utility.
- Using `spc` you can run `setup-php` on both `i386` and `amd64` containers in GitHub Actions.
...
jobs:
  run:
    ...
    container: shivammathur/node:latest-${{ matrix.tag }}
    strategy:
      matrix:
        tag: ["amd64", "i386"]
    steps:
